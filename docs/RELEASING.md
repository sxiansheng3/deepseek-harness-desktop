...
4. Restore the pinned Apple Silicon Node distribution under `vendor/node` and run `DSH_DESKTOP_NOTARY_PROFILE=<saved-profile> npm run dist:mac` with the Developer ID identity and validated `notarytool` Keychain profile available. The profile name is not a secret; its stored credentials are.
